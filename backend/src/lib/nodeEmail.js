...
export const MailWelcome = async (email, name, otpCode) => {
  try {
    ...
    const info = await resend.emails.send({
      from: "ShareGifts Team <onboarding@resend.dev>", // Use test email for now or configured domain
      to: email,
      subject: "Welcome to ShareGifts!",
      html: htmlContent,
    });
    console.log("Welcome Email sent: %s", info.id);
    return info;
  } catch (error) {
    console.log("Error sending welcome email:", error);
